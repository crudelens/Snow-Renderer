...
class Funcs:
    def __init__(self,directorycheck:str,MYDIR:str,renderformat:str,renderdirectory:str) -> None:
    ...
    def renderexec(self):
        ...
        if self.directorycheck==self.MYDIR:
            mainwindowInt.close()
            subprocess.run(["blender","-b", f"{self.MYDIR}/snowflake.blend", "-P", f"{self.MYDIR}/main.py", "-F", f"{self.renderformat}", "--render-output", f"{self.renderdirectory}"])
        else:
            mainwindowInt.close()
            subprocess.run(["./blender","-b", f"{self.MYDIR}/snowflake.blend", "-P", f"{self.MYDIR}/main.py", "-F", f"{self.renderformat}", "--render-output", f"{self.renderdirectory}"])
...
Funcsrunner=Funcs(f"{pathlib.Path(__file__).parent.absolute()}",f"{pathlib.Path(__file__).parent.absolute()}","PNG", "//rendered_output")
