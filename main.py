...
finhex.close()
finfilesno = open(f"{subloc}/subscripts/svg_filesno.txt", "rt")
nooffiles=int(finfilesno.read())
finfilesno.close()
def getdir():
    for i in range(0,nooffiles+1):
        if pathlib.Path(f"{subloc}/subscripts/SVG DIRECTORIES/svg_dir{i}.txt").is_file():
            finsvg = open(f"{subloc}/subscripts/SVG DIRECTORIES/svg_dir{i}.txt", "rt")
            path1=finsvg.read()
            finsvg.close()
            print("Should break loop")
            break
    return path1
print(getdir())
filepath=getdir()
print(filepath)
bpy.ops.import_curve.svg(filepath=filepath, filter_glob='*.svg')
...
for j in range(0,nooffiles+1):
        if pathlib.Path(f"{subloc}/subscripts/SVG DIRECTORIES/svg_dir{j}.txt").is_file():
            renderdir= finrenderdir.read() + f"/snowflake{j}"
            finrenderdir.close()
            break
